# from https://github.com/Hzzone/Precipitation-Nowcasting
...
def wrap(input, flow):
    B, C, H, W = input.size()
    # mesh grid
    xx = torch.arange(0, W).view(1, -1).repeat(H, 1).to('cuda')
    yy = torch.arange(0, H).view(-1, 1).repeat(1, W).to('cuda')
...
#F.leaky_relu(input, negative_slope=self.negative_slope, inplace=self.inplace)
...
class TrajGRU(BaseConvRNN):
    ...
    def forward(self, inputs=None, states=None, seq_len=10):
        ...
        return self.swap2(torch.stack(outputs)), next_h

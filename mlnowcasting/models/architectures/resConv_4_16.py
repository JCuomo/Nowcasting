...
def upsamplingLast(in_channels, out_channels, typ='all'):
    if typ == 'res':
        k=(3,4,4)
        s=(1,2,2)
    elif typ == 'seq':
...
class Nowcasting(nn.Module):
    def __init__(self, n_filter, dropout):
        ...
        self.e_conv2 = downsampling(in_channels=n_filter*2, out_channels=n_filter*3, typ='res')
        self.e_conv3 = downsampling(in_channels=n_filter*3, out_channels=n_filter*4, typ='res')
        
        self.d_conv3 = upsampling(in_channels=n_filter*4, out_channels=n_filter*3, typ='res')
        self.d_conv2 = upsampling(in_channels=n_filter*3, out_channels=n_filter*2, typ='res')
        self.d_conv1 = upsampling(in_channels=n_filter*2, out_channels=n_filter)
        self.d_conv0 = upsampling(in_channels=n_filter, out_channels=n_filter)
        self.d_convL1 = upsampling(in_channels=n_filter, out_channels=n_filter, typ='seq')
        self.d_convL0 = upsamplingLast(in_channels=n_filter,   out_channels=1, typ='seq')
    ...
    def forward(self, x):
        ...
        d0 = self.d_conv0(d1+e1)
        if debug: print(d0.shape)

        y1 = self.d_convL1(d0+x)
        if debug: print(y1.shape)
        y0 = self.d_convL0(y1)
        if debug: print(y0.shape)
        if debug: print('----------------')

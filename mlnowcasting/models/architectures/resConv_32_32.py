...
class Nowcasting(nn.Module):
    def __init__(self, n_filter, dropout):
        ...
        self.e_conv4 = downsampling(in_channels=n_filter*4, out_channels=n_filter*5, typ='seq')
        
        self.d_conv4 = upsampling(in_channels=n_filter*5, out_channels=n_filter*4, typ='seq')
    ...
    def forward(self, x):
        ...
        e5 = self.e_conv4(e4)
        if debug: print(e5.shape)
        
        d4 = self.d_conv4(e5)
        if debug: print(d4.shape)
        d3 = self.d_conv3(e4+e4)

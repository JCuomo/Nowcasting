...
# add extension "ar2v" to files
# bash addextenstion.sh <dir>
...
log_dir='log_addextension'
rm -rf $log_dir
mkdir $log_dir

list=$log_dir'/list_of_files.txt'
#get all files to process
find $1 -type f ! -name "*.*" -print > $list
...
do
	echo "machine: $m"
    ssh $m "nohup python cluster/addextension.py cluster/$list $i $files_per_machine > cluster/$log_dir/log_$m" &
	i=$((i+files_per_machine));
done
rest_of_files=$(( $n_files-$files_per_machine*$n_machines ))
echo $rest_of_files
python addextension.py $list $i $rest_of_files > $log_dir'/log_local' &
exit
